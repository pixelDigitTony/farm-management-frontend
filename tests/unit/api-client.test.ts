...
it("keeps public requests anonymous and never refreshes their sessions", async () => {
  ...
  server.use(
    ...
    http.post("*/api/auth/refresh", () => { refreshes++; return HttpResponse.json({ token: "new" }); }),
  );
  await expect(api("/public/test")).rejects.toMatchObject({ status: 401 });
  expect(refreshes).toBe(0); expect(tokenStore.get()).toBe("private-token");
});

it("coalesces concurrent refreshes", async () => {
  tokenStore.set("expired"); let refreshes = 0;
  server.use(
    http.get("*/api/resources/test", ({ request }) => request.headers.get("authorization") === "Bearer fresh"
      ? HttpResponse.json({ items: [] }) : HttpResponse.json({}, { status: 401 })),
    http.post("*/api/auth/refresh", () => { refreshes++; return HttpResponse.json({ token: "fresh" }); }),
  );
  await Promise.all([api("/resources/test"), api("/resources/test")]);
  expect(refreshes).toBe(1); expect(tokenStore.get()).toBe("fresh");
});
...
it("does not resurrect a session when logout occurs during refresh", async () => {
  ...
  expect(await pending).toBeNull(); expect(tokenStore.get()).toBeNull();
});
