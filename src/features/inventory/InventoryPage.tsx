...
export function InventoryPage() {
  ...
  const { inventory, lots, movements, accounts } = useInventoryQueries();
  const { data, isLoading, isError, error, refetch } = inventory;
  ...
  const receiveStock = useMutation({
    ...
    onSuccess: () => {
      void invalidateOperation(client, "receipt");
  ...
  const transferMeat = useMutation({
    ...
    onSuccess: () => {
      void invalidateOperation(client, "inventory");
